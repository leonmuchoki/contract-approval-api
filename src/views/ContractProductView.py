...
def create():
	...
	data, error = contract_product_schema.load(req_data)
	if error:
		return custom_response(error, 400)
	contract_prod = ContractProductModel(data)
	contract_prod.save()
	data = contract_product_schema.dump(contract_prod).data
...
def get_all():
	...
	data = contract_product_schema.dump(contract_products, many=True).data
